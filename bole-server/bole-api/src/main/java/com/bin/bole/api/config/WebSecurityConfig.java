...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    public void configure(WebSecurity web) throws Exception {
        web.ignoring().antMatchers("/css/**", "/js/**", "/index.html", "/img/**", "/fonts/**", "/favicon.ico", "/verifyCode","/bole/hello");
    }
    ...
    LoginFilter loginFilter() throws Exception {
        ...
        loginFilter.setAuthenticationSuccessHandler((request, response, authentication) -> {
                    ...
                    hr.setPassword(null);
                    Result ok = Result.success(ResultEnum.SUCCESS.getCode(), "登录成功!", hr);
        ...
        loginFilter.setAuthenticationFailureHandler((request, response, exception) -> {
                    response.setContentType("application/json;charset=utf-8");
                    PrintWriter out = response.getWriter();
                    Result respBean = Result.error(exception.getMessage());
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.addFilterAt(new ConcurrentSessionFilter(sessionRegistry(), event -> {
            ...
            resp.setStatus(401);
            PrintWriter out = resp.getWriter();
            out.write(new ObjectMapper().writeValueAsString(Result.error("您已在另一台设备登录，本次登录已下线!")));
            out.flush();
            out.close();
